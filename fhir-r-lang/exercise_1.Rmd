...
Now that we know a little bit about the server, let's query for all the patients. Again, there are two different approaches to doing this: using the `fhircrackr` library, or directly queryingthe server with `httr`. Let's see the `httr` approach first:
