---
title: "Exercise 1: Patients Prescribed Opioids (R version)"
output:
  html_document:
    df_print: paged
---
...
The CapabilityStatement is usually a very large resource so we'll focus on a few key elements. Namely, the FHIR version supported, the Resource formats supported, the number of RESTful Endpoints, and the endpoint types.
...
You can also do this directly with the `httr` library to manually make the HTTP request and load the results into R, but this takes some extra work:
...
```{r}
response <- httr::GET(
        url = str_interp("${fhir_server}/Patient/smart-1032702"),
...
Next we need to define the table description for `fhircrackr` to convert from the FHIR resources into data frames. Note that `fhircrackr` requests XML responses rather than JSON from the server. To understand the structure of the XML, let's look at the second resource returned by the server (the first one is missing some data that the others have):
