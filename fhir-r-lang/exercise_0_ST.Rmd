...
##?cs$Resources$typestr_c("FHIR Resources Supported: ", cs$Resources$type %>% length, "\n") %>% cat
```

```{r}
fhir_server <- "https://api.logicahealth.org/researchonfhir/open/"
request <- fhir_url(url = fhir_server, resource = "Patient", parameters = list("_id"="smart-1032702"))
patient_bundle1 <- fhir_search(request = request)
patient_bundle1
request2 <- fhir_url(url = fhir_server, resource = "Patient", parameters = list("_id"="BILIBABY"))
patient_bundle2 <- fhir_search(request = request2)
patient_bundle2
```
...
patient_bundles <- fhir_search(request=request, max_bundles=1, verbose = 1)
patient_bundles
```
...
```{r}
ids <- c("smart-1032702", "smart-1272431")
id_string <- paste(ids, collapse = ",")
body <- fhir_body(content = list("identifier" = id_string, "_revinclude" = "Observation:patient"))

url <- fhir_url(url = fhir_server, resource = "Patient")
...
```{r}
df_patient <- fhir_crack(bundles = patient_bundles, design = table_desc_patient, verbose = 0)
df_patient
```
